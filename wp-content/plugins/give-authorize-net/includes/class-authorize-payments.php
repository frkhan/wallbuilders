...
class Give_Authorize_Payments {
	...
	public function process_payment( $donation_data ) {
		...
		try {
			...
			if ( $response->isSuccessful() && $response->isApproved() ) {
			...
			} elseif ( $response->isSuccessful() && ! $response->isApproved() ) {
				if ( isset( $payment_id ) ) {
					give_set_payment_transaction_id( $payment_id, $response->transactionResponse->transId );
					// Not approved. An error with the payment.
					$error_message = isset( $response->transactionResponse->errors[0]->errorText ) ?  $response->transactionResponse->errors[0]->errorText : __('Authorize.net transaction declined by the fraud filter. Your transaction is under review.', 'give-authorize');
				...
				if ( isset( $response->transactionResponse->errors[0]->errorCode ) && '17' === $response->transactionResponse->errors[0]->errorCode ) {
