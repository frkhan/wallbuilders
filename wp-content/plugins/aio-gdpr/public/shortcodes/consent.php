...
function AIOGDPR_ConsentShortcode(){
    ob_start();
	?>
		...
		<div class="container">
			...
			<?php if(AIOGDPR_Settings::get('privacy_policy_overview') != ''): ?>
				<div class="row">
					<div class="column">
						<h2>Overview</h2>
						<p><?= AIOGDPR_Settings::get('privacy_policy_overview') ?></p>
					</div>
				</div>
				<hr>
			<?php endif; ?>
			...
			<div class="row">
				<div class="column">
					...
					<?= apply_filters('the_content', AIOGDPR_Settings::get('privacy_policy')) ?>
				</div>
			</div>
    	</div>
    </div>
...
add_shortcode('consent', 'AIOGDPR_ConsentShortcode');
