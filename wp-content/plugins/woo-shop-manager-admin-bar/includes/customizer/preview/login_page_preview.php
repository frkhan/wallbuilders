...
if ( is_multisite() ) {
	...
	if( !empty($image_path) ) {
		...
		$login_header_title = '<img src="'. $image_path.'">';
	} else {
		$login_header_url   = __( 'https://wordpress.org/', 'login-customizer' );
...
<?php do_action( 'login_header' ); ?>
	<div id="login">
		...
		<form name="loginform" id="loginform">
			<p>
				<label for="user_login"><?php _e( 'Username or Email Address', 'login-customizer' ); ?></label>
				<input type="text" name="log" id="user_login" class="input" value="" size="20" autocapitalize="off" />
			</p>
			<p>
				<label for="user_pass"><?php _e( 'Password', 'login-customizer' ); ?></label>
			...
			<p class="submit">
				<input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e( 'Log In', 'login-customizer' ); ?>" style="text-transform: capitalize;font-weight: 400;"/>
			</p>
		</form>
		<p id="nav">
			<?php
