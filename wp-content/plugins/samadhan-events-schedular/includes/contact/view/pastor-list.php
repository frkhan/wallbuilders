...
class PastorAdd extends UserCRUD
{
    public  function __construct(){

        add_shortcode('smdn_pastor_list','Samadhan\PastorAdd::get_contact_form_pastor_list');
    }

    // Contacts form pastor

    public static function get_contact_form_pastor_list($atts){
        ...
        foreach ($getUsers['results'] as $user){
            ...
            $body .=" <tr>
                   ...
                   <td>".get_user_meta($user->ID,'Salutation',true)."</td>
                   ...
                   <td>".get_user_meta($user->ID,'Church',true)."</td>
                   ...
                   <td>".(get_user_meta($user->ID,'PBAttendee',true) ? "Yes":"No")."</td>
                   <td>".(get_user_meta($user->ID,'SchedulingDB',true) ? "Yes":"No")."</td>
                   <td>".(get_user_meta($user->ID,'AAPastor',true) ? "Yes":"No")."</td>
                   <td>".(get_user_meta($user->ID,'ListPastor',true) ? "Yes":"No")."</td>
                   <td>".(get_user_meta($user->ID,'BRRPastor',true) ? "Yes":"No")."</td>
                   <td>".(get_user_meta($user->ID,'HISPastor',true) ? "Yes":"No")."</td>
                   <td>".(get_user_meta($user->ID,'NopId',true) ? "Yes":"No")."</td>
                   <td><a  style='font-size: 25px;margin-right: 10px;'  href=".home_url('/edit-pastor/?user_id='.$user->ID)."><span class='btn btn-small btn-info '>&#9998;</span></a><a style='font-size: 25px' href=".home_url('/pastor_list/?user_id='.$user->ID)."><span class='btn btn-small btn-danger'>&#10005;</span> </a></td>
        ...
        $outPut ="  
                             ...
                             <span style='color:#fff; font-size: 20px !important; margin-left: 10px;'> Pastor List </span>
