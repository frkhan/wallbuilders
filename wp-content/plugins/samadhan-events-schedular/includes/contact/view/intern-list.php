...
class InternAdd extends UserCRUD{

    public  function __construct(){

        add_shortcode('smdn_intern_list','Samadhan\InternAdd::get_intern_form_pastor_list');
    }


    public static function get_intern_form_pastor_list($atts){
        ...
        if(isset($_GET['user_edit']) && !empty($_GET['user_edit'])){
            ...
            $message=parent::edit_user_data($user_id);
        ...
        foreach ($getUsers['results'] as $user){
            ...
            $body .=" <tr>
                 ...
                 <td>".get_user_meta($user->ID,'Currentschool',true)."</td>
                 <td>".get_user_meta($user->ID,'Major',true)."</td>
                 <td>".get_user_meta($user->ID,'Occupations',true)."</td>
                 <td>".get_user_meta($user->ID,'Meetaclass',true)."</td>
                 <td><a style='font-size: 25px;margin-right: 10px;' href=".home_url('/edit-intern/?user_id='.$user->ID)."><span class='btn btn-small btn-info '>&#9998;</span></a><a style='font-size: 25px' href=".home_url('/intern_list/?user_id='.$user->ID)."><span class='btn btn-small btn-danger'>&#10005;</span> </a></td>
                 
          </tr>";
        }

        $outPut ="              <h3 style='color: red'>$show_message</h3>
