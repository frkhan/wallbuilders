...
class WC_AvaTax_Frontend {
	...
	protected function get_validate_address_button( $type = 'billing' ) {
		...
		return '<a class="wc_avatax_validate_address button" data-address-type="' . esc_attr( $type ) . '">' . esc_html( $label ) . '</a>';
	}
	...
	public function adjust_single_tax_total_html( $html ) {
		...
		$cart = WC()->cart;
		$taxes = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ? $cart->get_cart_contents_taxes() : $cart->taxes;
