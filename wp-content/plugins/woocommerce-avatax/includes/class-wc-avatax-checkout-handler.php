...
class WC_AvaTax_Checkout_Handler {
	...
	protected function set_product_taxes( WC_Cart $cart, $lines, $tax_included = false ) {
		...
		$subtotal   = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ? $cart->get_subtotal() : $cart->subtotal;
		$cart_taxes = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ? $cart->get_cart_contents_taxes() : $cart->taxes;
	...
	protected function set_fee_taxes( WC_Cart $cart, $lines ) {
		...
		foreach ( $lines as $line ) {
			...
			$fee_taxes = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ? $cart->get_fee_taxes() : $cart->taxes;
	...
	protected function set_shipping_taxes( WC_Cart $cart, $lines ) {
		...
		$cart_shipping_taxes = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ? $cart->get_shipping_taxes() : $cart->shipping_taxes;
