<div class="wrap aimtell signup">
    <div class="centerfloat">
        ...
        <div class="loginBox">
            ...
            <div id="register-step-one">
                <div class="heading">
                    ...
                    <span class="byline"> Get started with push notifications. </span>
                </div>
                Username: <input class="form-input" type="text" name="username">
                Password: <input class="form-input" type="password" name="password">
                <div class="clearfix">
                    <div class="pull-right">
                        <button id="registerSubmit-one" type="button" class="btn btn-primary">Create Account</button>
                    </div>
                </div>
            </div>
            ...
            <div id="register-step-two">
                <div class="heading">
                    <span class="title"> Almost Done! </span>
                    <span class="byline"> Please fill out the last bits of information below </span>
                </div>
                First Name: <input class="form-input" type="text" name="first_name">
                Last Name: <input class="form-input" type="text" name="last_name">
                Timezone: 
                <select class="timezonefield select2-offscreen" name="timezone" data-placeholder="Choose One" style="width:100%" required="" title="" tabindex="-1">
                ...
                <div class="row">
                    <div align="center" class="col-md-12 whitetext">
                        <input type="checkbox" id="agree" value="1">
                        <label class="whitetext" for="agree">I agree with the <a class="whitetext" target="_blank" href="https://aimtell.com/terms">Terms &amp; Conditions</a></label>
                    </div>
                </div>
                <div class="row">
                    <div align="center" class="col-md-12">
                        <button id="createUser" type="button" class="btn btn-primary">Create Account</button>
                    </div>
                </div>
            </div>
        ...
        <div align="center" class="login-text"> <span> Already a member? Click to login.</span> </div>
    </div>
</div>
...
<script>
    ...
    //user clicking to go to login
    jQuery(".login-text").click(function(event) {
         gotoUrl(window.location.href, {'page':'login'});
    });

    //submitting registration part one
    jQuery("#registerSubmit-one").click(function(event) {
        ...
        //check to make sure structure looks like email
        if(validateEmail(username) !== true){
            jQuery(".errorBox").text("Please enter a valid email format.");
            jQuery(".errorBox").show();
            return false;
        }

        jQuery("#registerSubmit-one").text("Checking username...")
        jQuery("#registerSubmit-one").prop("disabled", true);

        //check to make sure the username is free
        available = checkUsernameAvailability(username);
        jQuery.when(available).done(function(data){

            jQuery("#registerSubmit-one").text("Create Account")
            jQuery("#registerSubmit-one").prop("disabled", false);

            //if the user already exists, toss an error
            if(data.exists == true){
                jQuery(".errorBox").text("Username already exists.");
                jQuery(".errorBox").show();
                return false;
            }

            jQuery("#register-step-one").hide();
            jQuery("#register-step-two").show();
    ...
    //create user triggered, part 2 of registration
    jQuery("#createUser").click(function(event) {
        ...
        if(jQuery("input[name=first_name]").val().length < 1 || jQuery("input[name=last_name]").val().length < 1){
            jQuery(".errorBox").text("Please make sure you have filled out all the fields");
            jQuery(".errorBox").show();
            return false;
        }

        //check to see if user has approved terms
        if(jQuery("#agree").prop("checked") == false){
            jQuery(".errorBox").text("Please approve the terms and conditions");
            jQuery(".errorBox").show();
            return false;
        }

        //show loading 
        jQuery("#createUser").text("Creating Account... ")
        jQuery("#createUser").prop("disabled", true);

        //prepare the object
        params = {}

        //grab the information we need
        params.username = jQuery("input[name=username]").val();
        params.password = jQuery("input[name=password]").val();
        params.first_name = jQuery("input[name=first_name]").val()
        params.last_name = jQuery("input[name=last_name]").val();
        params.timezone = jQuery(".timezonefield option:selected").val();
        params.ref_code = jQuery("input[name=ref_code]").val();
        params.trial = 1;
        params.wp = 1;
        ...
        jQuery.when(result).done(function(data){
            ...
            //if fail
                ...
                //hide loading 
                jQuery("#createUser").text("Create Account")
                jQuery("#createUser").prop("disabled", false);
                //show error
                jQuery(".errorBox").text(data.message);
                jQuery(".errorBox").show();
            }
