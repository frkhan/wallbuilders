...
if ( ! class_exists( 'Give_Subscriptions_Renewals_Export' ) ) {
	...
	class Give_Subscriptions_Renewals_Export extends Give_Batch_Export {
		...
		public function set_properties( $post_data ) {
			...
			$this->form       = $post_data;
			$this->start_date = $post_data['give_renewal_subscriptions_start_date'];
			$this->end_date   = $post_data['give_renewal_subscriptions_end_date'];
		...
		public function get_data() {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				if (
					strtotime( $this->start_date ) < strtotime( $subscription->get_renewal_date() ) &&
					strtotime( $this->end_date ) > strtotime( $subscription->get_renewal_date() )
				) {
